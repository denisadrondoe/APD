...
void *thread_function(void *arg)
{
	...
	for (k = 0; k < log(N) + 1; k++) { //nr aproximativ de faze necesare pentru a sorta intreaga matrice
		// se sorteaza liniile pare crescator
		// se sorteaza liniile impare descrescator
		for (i = start; i < end; i++) { //fiecare thread sorteaza in paralel randurile care ii revin
			if (i % 2) {
		...
		for (i = start; i < end; i++) {  //coloanele 
			for (j = 0; j < L; j++) {
				 aux[j] = M[j][i]; //memoreaza coloana i
			}

			qsort(aux, L, sizeof(int), cmp); //sorteaza coloana

			for (j = 0; j < L; j++) {
				M[j][i] = aux[j]; //pune inapoi in matrice
			}
		}
