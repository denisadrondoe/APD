...
/* Thread functions: fiecare calculeaza cate un M */
...
int main(int argc, char *argv[])
{
	...
	/* pornim 7 thread-uri, cate unul pentru fiecare M */
	if (pthread_create(&t1, NULL, compute_M1, NULL) != 0) {
		perror("pthread_create M1");
		exit(1);
	}
	if (pthread_create(&t2, NULL, compute_M2, NULL) != 0) {
		perror("pthread_create M2");
		exit(1);
	}
	if (pthread_create(&t3, NULL, compute_M3, NULL) != 0) {
		perror("pthread_create M3");
		exit(1);
	}
	if (pthread_create(&t4, NULL, compute_M4, NULL) != 0) {
		perror("pthread_create M4");
		exit(1);
	}
	if (pthread_create(&t5, NULL, compute_M5, NULL) != 0) {
		perror("pthread_create M5");
		exit(1);
	}
	if (pthread_create(&t6, NULL, compute_M6, NULL) != 0) {
		perror("pthread_create M6");
		exit(1);
	}
	if (pthread_create(&t7, NULL, compute_M7, NULL) != 0) {
		perror("pthread_create M7");
		exit(1);
	}
	...
	/* asamblam matricea c (serial, dupa ce toate M* sunt gata) */
