...
int main (int argc, char *argv[])
{
    ...
    MPI_Comm_size(custom_group, &new_size); // Size of the new group.
    MPI_Comm_rank(custom_group, &new_rank); // Rank in the new group.
    ...
    // Receive the rank.
    MPI_Sendrecv(&old_rank, 1, MPI_INT, (new_rank + 1) % new_size, 0,
             &recv_rank, 1, MPI_INT, (new_rank - 1 + new_size) % new_size, 0,
